...
function imgt_build_tag_groups($wpit_admin_tags_groups, $wpit_admin_tags_source){
	...
	if (count($grpitems)) {
		$group_item = new StdClass;
		$group_item->group_name = (isset($default_group_name) ? $default_group_name : __('Others', 'imagetagger'));
		$group_item->group_tags = $grpitems;
		$tags_groups[] = $group_item;
	}
...
function print_tag_form($checked_tags, $admin_page = 0) {
	...
	foreach ($g_imgt_tag_taxonomy as $key=>$g_imgt_tag_taxonomy_item) {
		...
		$strout .= '<input type="checkbox" value=' . $g_imgt_tag_taxonomy_item->term_taxonomy_id . " name=tags[]" . ($checked? " checked" : "") . '> ' . 
			...
			_n('image associated to tag', 'images associated to tag', $g_imgt_tag_taxonomy_item->count, 'imagetagger') .
...
function admin_get_current_image($img_list, $reason) {
	...
	if (empty($img_list)) {
		...
		switch ($reason) {
			case 1:
				_e("This blog does not hold any image.", 'imagetagger'); echo "</p><p>";
				_e("Start to create posts or pages containing images before trying to tag images.", 'imagetagger');
				break;
			case 2:
				_e("Images are already all tagged.", 'imagetagger'); echo "</p><p>"; 
				_e('To modify tagging, go to the Image Explorer view and select "tagged images" or "all images".', 'imagetagger');
				break;
			case 3:
				_e("There is no tagged image yet.", 'imagetagger'); echo "</p><p>"; 
				_e('To start tagging, go to the Tag Editor view or select "untagged images" or "all images" in the Image Explorer view.', 'imagetagger');
				break;
		}
		echo "</p></strong></div>";
		return 0;
...
function admin_check_taxonomy_not_empty(){
	...
	if (empty($g_imgt_tag_taxonomy)) {
		echo '<div class="updated"><strong><p>';
		_e("No tag was found in this blog", 'imagetagger'); echo "</p><p>";
		_e("Start to create a list of tag that you will use to tag your images", 'imagetagger');
		echo "</p></strong></div>";
		return 0;				  
	}
	return 1;
...
function imgt_get_img_info($obj_id) {
	...
	$img_src = wp_get_attachment_image_src($obj_id, 'full');
	//print_ro($img_src);
	//$img_info->url = get_relative_path($img_src[0]);
	$img_info->url = $img_src[0];	// ex : http://www.photos-dauphine.com/wp-content/uploads/2009/09/sous-cochet-chemin-lauze.jpg

	$img_info->w = $img_src[1];
	$img_info->h = $img_src[2];
	
	//print "<br/>Image source : " . $img_info->url . "<br/>";
...
function imgt_taxonomy_update(){
	...
	if ( $count->total != $count->tagged + $count->untagged) {
		...
		if (!empty($img_orphean_id)) {
			echo "ImageTagger plugin detected image(s) referenced in the image taxonomy but not anymore in the posts table - Probably deleted" . ".<br/>";
			echo "The entries for image(s)" . " " . implode(', ', $img_orphean_id) . " " . "are removed from the image taxonomy table" . ".<br/>";
	...
	if ($wpit_admin_tags_source <= 2) {	// select only tags, or tags and categories
		...
		array_walk($g_imgt_tag_taxonomy_tags, imgt_walk_add_category, __('Tags', 'imagetagger'));
	}
	...
	if ($wpit_admin_tags_source >= 2) {	// only select categories, or tags and categories
		...
		array_walk($g_imgt_tag_taxonomy_cats, imgt_walk_add_category, __('Categories', 'imagetagger'));
	}
...
function imgt_update_post_tags($post_id, $update, &$update_required){
	...
	if (!$update) {
		...
		echo '=== <em>' . __('Post', 'imagetagger') . '</em> : <strong>' . get_the_title($post_id) . "</strong> ===<br/>";
		//echo '<em>' . _n('Original category', 'Original categories', count($cats), 'imagetagger') . ":</em> " . $original_cats . "<br/>";
		echo '<em>' . _n('Original tag', 'Original tags', count($current_tax), 'imagetagger') . ":</em> " . $original_tags . "<br/>";
		echo '<em>' . _n('New tag', 'New tags', count($current_tax), 'imagetagger') . ":</em> " . $new_tags . "<br/>";
...
function imgt_database_fix_post_revisions($fix_revisions_liststr) {
	...
	if (empty($fix_revisions_liststr)) {
		echo "<p><strong>>> " . __('Post revision cleanup : nothing to clean', 'imagetagger') . ".</strong><p><br/>";
		return;
	}

	$sql_query = 'DELETE FROM ' . $wpdb->posts . ' WHERE ID IN (' . $fix_revisions_liststr . ')';
	
	printf('<p><strong>>> ' . __('Cleaning up table %s from post revisions', 'imagetagger') . '<br/>', $wpdb->posts);
	printf('>> ' . __('Running SQL : %s', 'imagetagger') . ' ...<br/>', $sql_query);
	run_mysql_query($sql_query);
	echo ">> " . __('Done', 'imagetagger') . ".</strong></p>";
}
...
function imgt_database_fix_post_attachments($fix_attachments_liststr) {
	...
	if (empty($fix_attachments_liststr)) {
		echo "<p><strong>>> " . __('Post attachment cleanup : nothing to clean', 'imagetagger') . ".</strong><p><br/>";
		return;
	}

	$sql_query = 'DELETE FROM ' . $wpdb->posts . ' WHERE ID IN (' . $fix_attachments_liststr . ')';
	
	printf('<p><strong>>> ' . __('Cleaning up table %s from unused attachments', 'imagetagger') . '<br/>', $wpdb->posts);
	printf('>> ' . __('Running SQL : %s', 'imagetagger') . ' ...<br/>', $sql_query);
	run_mysql_query($sql_query);
	echo ">> " . __('Done', 'imagetagger') . ".</strong></p>";
...
function imgt_database_fix_post_taxonomy($fix_posttax_liststr) {
	...
	if (empty($fix_posttax_liststr)) {
		echo "<p><strong>>> " . __('Post taxonomy cleanup : nothing to clean', 'imagetagger') . ".</strong><p><br/>";
		return;
	}

	$sql_query = 'DELETE FROM ' . $wpdb->term_relationships . ' WHERE object_id IN (' . $fix_posttax_liststr . ')';
	
	printf('<p><strong>>> ' . __('Cleaning up table %s from entries referring to posts not having anymore an entry in the posts table', 'imagetagger') . '<br/>', $wpdb->wp_term_relationships);
	printf('>> ' . __('Running SQL : %s', 'imagetagger') . ' ...<br/>', $sql_query);
	run_mysql_query($sql_query);
	echo ">> " . __('Done', 'imagetagger') . ".</strong></p>";
...
function imgt_database_fix_image_taxonomy($fix_imgtax_liststr) {
	...
	if (empty($fix_imgtax_liststr)) {
		echo "<p><strong>>> " . __('Image taxonomy cleanup : nothing to clean', 'imagetagger') . ".</strong><p><br/>";
		return;
	}

	$sql_query = 'DELETE FROM ' . TERM_REL_IMG . ' WHERE object_id IN (' . $fix_imgtax_liststr . ')';
	
	printf('<p><strong>>> ' . __('Cleaning up table %s from entries referring to images not having anymore an entry in the posts table', 'imagetagger') . '<br/>', TERM_REL_IMG);
	printf('>> ' . __('Running SQL : %s', 'imagetagger') . ' ...<br/>', $sql_query);
	run_mysql_query($sql_query);
	echo ">> " . __('Done', 'imagetagger') . ".</strong></p>";
...
function imgt_database_audit(&$fix_revisions_liststr, &$fix_attachments_liststr, &$fix_posttax_liststr, &$fix_imgtax_liststr, $pure_audit){
	...
	echo ($pure_audit ? "<br/>" : '') . __('Check below the possibly detected database inconsistencies', 'imagetagger') . '.<br/>';
	echo __('If you go for the cleanup, the process will delete some entries in the posts, post taxonomy and image taxonomy table(s)', 'imagetagger') . '.<br/>';
	echo '<span style="color:#00F">' . __('For this reason it is mandatory to backup your database before to be able to revert to the original database setup if needed', 'imagetagger') . '</span>.<br/>';
	echo __('In any case, read entirely the list of impacted entries shown in the analysis below before deciding to cleaning up', 'imagetagger') . '.<br/><br/>';
	echo __('Given the dependencies, execute the following sequence partially or totally, assuming you have the proper understanding of the consequences on your database', 'imagetagger') . '.<br/>';
	echo '<span style="color:#00F">' . __('If any doubt, do not run the cleanup sequence you are not clear with', 'imagetagger') . '.</span><br/><br/>';
	
	printf('1 - ' . __('Left-over post revisions (table %s)', 'imagetagger') . '<br/>', $wpdb->posts);
	echo '<p style="margin:0;padding:0 0 5px 24px;font-style:italic">' . 
		__('This step does not induce any risk of data loss. It will erase posts revisions automatically created and not needed anymore', 'imagetagger') . '.</p>';
		
	printf('2 - ' . __('Media library elements not included in any post content (table %s)', 'imagetagger') . '<br/>', $wpdb->posts);
	echo '<p style="margin:0;padding:0 0 5px 24px;font-style:italic">' . 
		__('This step might present risk of data loss and requires proper understanding and judgment ; it will remove attachments from the table if this attachment does not appear anywhere in any of the posts content. Do not use it if you created a media library with items not included in the posts, and still want to keep this library unchanged', 'imagetagger') . '.</p>';
		
	printf('3 - ' . __('Left-over post taxonomies (table %s)', 'imagetagger') . '<br/>', $wpdb->term_relationships);
	echo '<p style="margin:0;padding:0 0 5px 24px;font-style:italic">' . 
		__('This step does not induce any risk of data loss. It will erase from the post taxonomy table post IDs still tagged but not present anymore in the posts table as attachment entries', 'imagetagger') . '.</p>';

	printf('4 - ' . __('Left-over image taxonomies (table %s)', 'imagetagger') . '<br/>', TERM_REL_IMG);
	echo '<p style="margin:0;padding:0 0 5px 24px;font-style:italic">' . 
		__('This step does not induce any risk of data loss. It will erase from the image taxonomy table image IDs still tagged but not present anymore in the posts table as attachment entries', 'imagetagger') . '.</p>';

	printf('5 - ' . __('Information only : Images remaining untagged (table %s)', 'imagetagger') . '<br/>', TERM_REL_IMG);
	echo '<p style="margin:0;padding:0 0 10px 24px;font-style:italic">' . 
		__('This analysis provides you the list of untagged images. To fix situation where images would remain untagged, go to the ImageTagger Tag Editor and complete the job', 'imagetagger') . '.</p>';

	echo __('The benefit of this cleanup is to reduce your database to the only required information, therefore improving site reactivity and decreasing required processing power', 'imagetagger') . '.<br/><br/>';
	
	echo "<h3>" . __('Analysis result', 'imagetagger') . ' : </h3>';
	
	//
	//	1 - Search post revisions
	//
	printf('<p><strong>1 - ' . __('Left-over post revisions (table %s)', 'imagetagger') . '</strong></p>', $wpdb->posts);
	...
	if (($mysql_result = run_mysql_query($sql_query)) !== "SQL_EXEC_ERROR") {
		...
		if (empty($mysql_result))
			echo '<p class="post_list">' . __('No left-over post revision found - No cleanup needed', 'imagetagger') . '.</p>';
		else {
	...
	//
	printf('<p><strong>2 - ' . __('Media library elements not included in any post (table %s)', 'imagetagger') . '</strong></p>', $wpdb->posts);
	...
	if (($mysql_result = run_mysql_query($sql_query)) !== "SQL_EXEC_ERROR") {
		...
		if ($cleanup_needed) {
			...
			echo '<p class="post_list">' . __('No orphean attachment found - no cleanup needed', 'imagetagger') . '.</p>';
	}
	
	//
	//	3 - Left-over post taxonomies : taxonomies provided for a given post id not existing anymore in table posts
	//
	printf('<p><strong>3 - ' . __('Left-over post taxonomies (table %s)', 'imagetagger') . '</strong></p>', $wpdb->term_relationships);
	...
	if (($mysql_result = run_mysql_query($sql_query)) !== "SQL_EXEC_ERROR") {
		...
		if ($cleanup_needed) {
			...
			echo '<p class="post_list">' . __('No left-over post taxonomy found - no cleanup needed', 'imagetagger') . '.</p>';
	}
	...
	//
	printf('<p><strong>4 - ' . __('Left-over image taxonomies (table %s)', 'imagetagger') . '</strong></p>', TERM_REL_IMG);
	...
	if (($mysql_result = run_mysql_query($sql_query)) !== "SQL_EXEC_ERROR") {
		...
		if ($cleanup_needed) {
			...
			echo '<p class="post_list">' . __('No left-over image taxonomy found - no cleanup needed', 'imagetagger') . '.</p>';
	}
		
	//
	//	5 - Untagged images
	//
	printf('<p><strong>5 - ' . __('Information only : Images remaining untagged (table %s)', 'imagetagger') . '</strong></p>', TERM_REL_IMG);
	...
	if (($mysql_result = run_mysql_query($sql_query)) !== "SQL_EXEC_ERROR") {
		...
		if (!$cleanup_needed) 
			echo '<p class="post_list">' . __('All the images are tagged, you made it well !', 'imagetagger') . '</p>';
	}
