<?php
/*
Plugin Name: ImageTagger
Plugin URI: http://www.photos-dauphine.com/wp-mediatagger-plugin
Description: This plugin is now obsoleted by <a href="http://wordpress.org/extend/plugins/wp-mediatagger/">WP MediaTagger</a>. <strong>IMPORTANT - TO AVOID LOOSING THE TAGGING ALREADY DONE :</strong> before installing MediaTagger, <a href="http://wordpress.org/extend/plugins/wp-mediatagger/installation/"><strong>DEACTIVATE the ImageTagger plugin</strong></a>. Then proceed with the MediaTagger install as explained on the <a href="http://wordpress.org/extend/plugins/wp-mediatagger/installation/">MediaTagger installation page</a>.
...
load_plugin_textdomain('imagetagger', 'wp-content/plugins/' . $wpit_dir, $wpit_dir);


////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
//	Init ImageTagger plugin
//
...
//
function wpit_menu(){
    include 'imagetagger-admin.php';
}

//////////////////////
//
function wpit_admin_actions(){
    add_options_page("ImageTagger", "ImageTagger", 10, "imagetagger", "wpit_menu");
}
 
add_action('activate_wp-imagetagger/imagetagger.php', 'wpit_install');
add_action('admin_menu', 'wpit_admin_actions');


////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
//	Init ImageTagger widget
//
function wpit_widget($args) {
	...
	// Insert the ImageTagger tag cloud
...
//////////////////////
//
function init_imagetagger_widget(){
	wp_register_sidebar_widget("ImageTagger", "ImageTagger", "wpit_widget", array('description'=>__('Display your ImageTagger tag cloud in the sidebar. Before that, you need to have properly tagged your images in the ImageTagger plugin Admin Panel and have as well setup a result page that you will use as your tag cloud target page', 'imagetagger')));     
	register_widget_control('ImageTagger', 'wpit_widget_control', null, null);
}
 
add_action("plugins_loaded", "init_imagetagger_widget");
...
function wpit_multisort_insert($result_page_url='', $num_tags_displayed = '', $font_size_min = '', $font_size_max = '', 
	...
	echo '<em>' . __('The direct PHP call to the ImageTagger plugin core function wpit_multisort_insert() is deprecated starting version 2.5.2 of this plugin.', 'imagetagger') . '</em><br/><br/>';
	echo __('It is finally disabled starting version 2.5.4.5 and needs to be replaced by its shortcode equivalent. This does not require anymore running PHP in your page.', 'imagetagger') . ' ';
	echo __('That\'s a very simple modification, please refer for this purpose to the WP ImageTagger ', 'imagetagger') . '<a href="http://wordpress.org/extend/plugins/wp-imagetagger/installation/">' . 
		__('installation guide', 'imagetagger') . '</a>' . __(' and ', 'imagetagger') . '<a href="http://wordpress.org/extend/plugins/wp-imagetagger/faq/">' . 
		__('FAQ', 'imagetagger');
	echo '.</p>';
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	if ($is_search_mode_switchable) { 
		$strout .= '<div style="clear:both;font-size:0.9em;padding:4px;margin:0;background-color:#' . $wpit_admin_background_color . '"><div style="float:right;color:#AAA;padding-right:5px;letter-spacing:1pt;font-variant:small-caps"><em>' . __('Search display', 'imagetagger') . '</em></div><div style="padding-left:5px">';
		if ($search_mode!=1) $strout .= '<a href="' . $result_page_url . '" onClick="' . $search_form_prefix . 'post_submit(\'link_triggered\',\'11\');return false" title="' .
			__('Display the search form as a tag cloud', 'imagetagger') . '">';
		$strout .= __('Tag cloud', 'imagetagger');
		if ($search_mode!=1) $strout .= '</a>';
		$strout .= ' &nbsp;';
		if ($search_mode!=3) $strout .= '<a href="' . $result_page_url . '" onClick="' . $search_form_prefix . 'post_submit(\'link_triggered\',\'13\');return false" title="' . __('Display the search form as a check boxes form', 'imagetagger') . '">';
		$strout .= __('Form', 'imagetagger');
		if ($search_mode!=3) $strout .= '</a>';
		$strout .= ' &nbsp;';
		if ($search_mode!=2) $strout .= '<a href="' . $result_page_url . '" onClick="' . $search_form_prefix . 'post_submit(\'link_triggered\',\'12\');return false" title="' . __('Display the search form as a combination of the tag cloud and check boxed form', 'imagetagger') . '">';
		$strout .= __('Combined', 'imagetagger'); 
	...
	if ($search_mode > 0 && $search_mode <= 2) { // Display tag cloud
		...
		foreach ($tax_tab as $tax){ 
			...
			$strout .= '<a href="' . $result_page_url . '" style="font-size:' . round($font_scale*$tax->count + $font_size_min, 1) . 'pt;line-height:110%;text-decoration:none;' .
				...
				' title="' . $tax->count . ' ' . _n('occurence', 'occurences', $tax->count, 'imagetagger') . '">' . $tax->name . '</a> ';
		}	// if ($search_mode <= 2)
		$strout .= '</p>';
	} // end tag cloud
	...
	if (empty($tax_id_list) ) {	// case no tag selected
		if ($search_mode == 0)
			$strout .= '<em>'. __('None of the selected tag(s) match existing tags. The image search URL should be composed as http://www.mysite.com/library?tags=tag1+tag2+...+tagN, where http://www.mysite.com/library is the search result page. Check the spelling of the tag slugs', 'imagetagger') . '</em> : <strong>' . $_GET['tags'] . '</strong>';
		else if ($search_mode == 2)
			$strout .= '<em>'. __('You can search an image by theme either with the tag cloud above or selecting appropriate keywords below and clicking OK.', 'imagetagger') . '</em>';
		else if ($search_mode == 3)
			$strout .= '<em>'. __('You can search an image by theme by selecting appropriate keywords below and clicking OK.', 'imagetagger') . '</em>';
	} else {
		// Tags selected - search must be started
		$tagsSelected = 1;
		//printf(_n('Theme matched', 'Themes matched', sizeof($_POST['tags']), 'imagetagger')) ; 
		$strout .= '&raquo;<strong> ';
		...
		if (!$num_img_found) {
			$strout .= '<i>' . __('no image found matching this criteria list', 'imagetagger') . '</i><br/>';
		} else {
			if ($num_img_stop > $num_img_found)
				$num_img_stop = $num_img_found;
				
			$strout .= '<i>' . $num_img_found . ' ';
			$strout .= _n('image found', 'images found', $num_img_found, 'imagetagger'); 
			$strout .= '</i><br/>&nbsp;<br/>';
...
if ($is_result_mode_switchable) {
	$strout .= '<div style="clear:both;font-size:0.9em;padding:4px;margin:0;background-color:#' . $wpit_admin_background_color . '"><div style="float:right;color:#AAA;padding-right:5px;letter-spacing:1pt;font-variant:small-caps"><em>' . __('Results display', 'imagetagger') . '</em></div><div style="padding-left:5px">';
	if ($result_mode != 1) $strout .= '<a href="' . $result_page_url . '" onClick="post_submit(\'link_triggered\',\'21\');return false" title="' . __('Display the results as an image gallery', 'imagetagger') . '">';
	$strout .= __('Gallery', 'imagetagger');
	if ($result_mode != 1) $strout .= '</a>';
	$strout .= ' &nbsp;'; 
	if ($result_mode != 2) $strout .= '<a href="' . $result_page_url  .'" onClick="post_submit(\'link_triggered\',\'22\');return false" title="' . __('Display the results as an itemized image list', 'imagetagger') . '">';
	$strout .= __('Itemized', 'imagetagger');
	if ($result_mode != 2) $strout .= '</a>';
	$strout .= ' &nbsp;';
	if ($result_mode != 3) $strout .= '<a href="' . $result_page_url . '" onClick="post_submit(\'link_triggered\',\'23\');return false" title="' .  __('Display the results as an image title list', 'imagetagger') . '">';
	$strout .= __('Titles', 'imagetagger');
					...
					case 1:	// gallery
						if ($wpit_result_display_optimize_xfer) {		// resize image before transfer for faster display
							$strout .= '<a href="' . ($result_mode==1 && $link_to_post ? $img_info->post_URI : $img_info->url) .
								'" title="' . $img_tooltip . '"><img src="' . $thumb_url . '?s=' . $img_info->url . '&w=' . $img_w . '&h=' . $img_h . 
							...
							$strout .= '<a href="' . ($result_mode==1 && $link_to_post ? $img_info->post_URI : $img_info->url) .
								'" title="' . $img_tooltip . '"><img src="' . $img_info->url . '" width="' . 
					...
					case 2:	// image list
						if ($wpit_result_display_optimize_xfer) {		// resize image before transfer for faster display
							$strout .= '<p style="padding: 10px 0 0 0;margin:0">' . $img_obj->post_title . 
								' (<a href="'. $img_info->post_URI . '" title="'  . __('Go to page', 'imagetagger') . '">' . $img_info->post_title. '</a>)<br/>' .
								'<a href="' . $img_info->url . '" title="' . $img_tooltip . '"><img src="' . $thumb_url . '?s=' . $img_info->url . '&w=' . $img_w . '&h=' . 
								$img_h . '" width="' . $img_w . '" height="' . $img_h . '" alt="' . $img_tooltip . '"></a></p>'; 
						} else {
							$strout .= '<p style="padding: 10px 0 0 0;margin:0">' . $img_obj->post_title . 
								' (<a href="'. $img_info->post_URI . '" title="'  . __('Go to page', 'imagetagger') . '">' . $img_info->post_title. '</a>)<br/>' .
								'<a href="' . $img_info->url . '" title="' . $img_tooltip . '"><img src="' . $img_info->url . '" width="' . 
								$img_w . '" height="' . $img_h . '" alt="' . $img_tooltip . '"></a></p>'; 
						}
						break;
					case 3:	// title list
						$strout .= '<p style="padding: 2px 0 0 0;margin:0"><a href="'. $img_info->post_URI . '" title="'  . __('Go to page', 'imagetagger') . '">' . 
						$img_info->post_title. '</a> : ' . '<a href="' . $img_info->url . '" title="' . __('Display image', 'imagetagger') . '">' . 
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	if ($display_pagination) { 
		...
		$strout .= sprintf(_n('image %d to %d', 'images %d to %d',  $num_img_found, 'imagetagger'), (int)($num_img_start+1), $num_img_stop) . ') &nbsp;&nbsp;</em>';
		if ($num_img_start > 0) $strout .= '<a href="' . $result_page_url . '" onClick="post_submit(\'link_triggered\',\'30\');return false" title="' . __("Click to display previous page",'imagetagger') . '">';
		$strout .= '&laquo; ' . __('previous', 'imagetagger') . ($num_img_start > 0 ? '</a>' : '') . '&nbsp;';
		if ($num_img_stop < $num_img_found) $strout .= '<a href="' . $result_page_url . '" onClick="post_submit(\'link_triggered\',\'31\');return false" title="' . __("Click to display next page",'imagetagger') . '">';
		$strout .= __('next', 'imagetagger') . ' &raquo;' . ($num_img_stop < $num_img_found ? '</a>' : '') . '</p>';
	}	// if ($display_pagination)
	if ($search_mode >= 2) {	// form
		...
		$strout .= '</div><div class="submit" style="clear:both;padding-top:15px;text-align:center"><input type="submit" value="OK" name="search" style="width:75px"></div>';
	...
	if (admin_get_option_safe('wpit_admin_credit', WPIT_ADMIN_INIT_CREDIT))
		$strout .= '<div style="clear:both;float:right;font-size:0.7em;padding:5px 10px 0 0"><a href="http://www.photos-dauphine.com/wp-imagetagger-plugin" title="' . 
			__('Offer an image search engine to your blog with', 'imagetagger') . ' WP ImageTagger ' . $WPIT_SELF_VERSION_STABLE . '"><em>ImageTagger</em></a></div>';
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
//
//	Define ImageTagger shortcode
//
// [imagetagger attributes]
//
...
function wpit_run_shortcode( $content ) {
	...
	add_shortcode( 'imagetagger', 'imgt_multisearch_shortcode_func');
...
function imgt_img_caption_shortcode($attr, $content = null) {
	...
	$href_su = ' href="/wp-admin/options-general.php?page=imagetagger&id='. $img_id . '"';
...
function myplugin_plugin_action_links($links, $file) {
    ...
    if ($file == $this_plugin) {
        ...
        $settings_link = '<a href="' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=imagetagger">' . __('Settings', 'imagetagger') . '</a>';
...
function set_plugin_meta($links, $file) {
	...
	if ($file == $plugin) {
		return array_merge(
			$links,
			array( sprintf( '<a href="https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=7T4VR9LD7NG6G">' . __('Donate', 'imagetagger') . '</a>' ))
		);
	}
	return $links;
}
